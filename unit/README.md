...
- `test_runner_console.cpp` - консольное приложение с псевдографическим интерфейсом
...
### Стандартный запуск (через gtest)
...
### Консольное приложение с псевдографическим интерфейсом

Рекомендуемый способ запуска тестов с красивым выводом результатов:
...
- Красивый псевдографический интерфейс с цветами
- Детальная статистика по тестам и тестовым наборам
- Интерактивное меню после завершения тестов:
  - `[R]` - Rebuild tests (пересборка тестов начисто: `make clean && make`)
  - `[Q]` - Quit (выход)

**Аргументы командной строки:**

```bash
# Запуск без интерактивного меню (старое поведение)
./test_runner_console --no-menu

# Только пересборка без запуска тестов
./test_runner_console --rebuild
```
...
- **LinkState_FirstPacket_EstablishesLink**: Переход из link down в link up
- **LinkState_RegularPackets_MaintainsLink**: Сохранение связи при регулярных пакетах
...
`test_runner_console` предоставляет:
- Псевдографический интерфейс с цветами и таблицами
- Детальную статистику по тестам
- Интерактивное меню для пересборки тестов
- Удобный просмотр результатов

Рекомендуется использовать `test_runner_console` вместо стандартного `test_runner` для лучшего опыта работы с тестами.
